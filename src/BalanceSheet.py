...
class BalanceSheet:
    def __init__(self, stockid, year, date):
        ...
        web = 'http://stockdata.stock.hexun.com/2008/zcfz.aspx?stockid=' + stockid + '&accountdate=' + temp_date
        msg = get_page(web)
        ...
        for i in range(len(asset_sheet_content)):
            self.data[asset_sheet_content[i]] = self.data_list[i]
        ...
        for wholestr in p:
            ...
            for year in range(startyear, finalyear + 1):
                ...
                if temp_date not in find_list:
                    continue
                new_sheet = BalanceSheet(stockid=stockid, year=year, date=date)
